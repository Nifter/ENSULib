...
describe UsersController do
  ...
  describe "GET 'index'" do
    ...
    describe "for signed-in users" do
      ...
      before(:each) do
        @user = Factory(:user)
        test_sign_in(@user)
        Factory(:user, :email => "anothertester@example.com")
        Factory(:user, :email => "anothertester2@example.com")
  ...
  describe "GET 'show'" do
    ...
    it "should have a profile image" do
      get :show, :id => @user
      # looks for the img tag between an h1 tag, with a class gravatar
      response.should have_selector('h1>img', :class => "gravatar")
    end

    it "should show the user's books" do
      book1 = Factory(:book, :user => @user, :borrowed_at => Time.now,
                             :title => "Book1")
      book2 = Factory(:book, :user => @user, :borrowed_at => Time.now,
                             :title => "Book2")
      get :show, :id => @user
    ...
    it "should paginate books" do
      35.times { Factory(:book, :user => @user, :borrowed_at => Time.now) }
      get :show, :id => @user
      response.should have_selector('div.pagination')
    end
  end
  ...
  describe "DELETE 'destroy'" do
    ...
    describe "as an admin user" do
      ...
      before(:each) do
        @admin = Factory(:user, :email => "admin@example.com", :admin => true) # Can set admin to true here because Factories bypass attr_accessible
        test_sign_in(@admin)
      end
